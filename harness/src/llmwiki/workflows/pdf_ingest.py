...
def build_planned_write_workflow(
    store: WikiStore,
    today: str,
    ...
    seen: set[str] = set()
    read_tracker = None if planned_write.page_metadata.page_kind == "source" else seen
    tools = [
        read_page_tool(store, read_tracker=seen),
        planned_write_page_tool(
            store,
        ...
        finish_tool(
            "finish_planned_write",
            "Finish after the exact planned target page has been written.",
        ),
    ]
    return Workflow(
        name="pdf-planned-write",
        description="Execute one planned page write from a global PDF ingest plan.",
        tools={t.name: t for t in tools},
        required_steps=["write_page"],
        terminal_tool="finish_planned_write",
        system_prompt_template=prompts.PLANNED_WRITE_TEMPLATE,
    )
