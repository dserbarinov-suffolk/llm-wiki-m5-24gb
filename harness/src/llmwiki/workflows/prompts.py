...
LINT_TEMPLATE = _BASE + (
    ...
    "- Lint cannot rewrite whole pages. Use only the constrained repair "
    "tools available in this workflow.\n"
    "- Before calling a repair tool for a page_id, read_page that same "
    "page_id so the read-before-mutate guardrail is satisfied.\n"
    "- An orphan page is fixed by add_related_link on a related existing "
    "source page: page_id is the page that should carry the link, "
    "target_page_id is the orphan.\n"
    "- A broken [[link]] on a manual page may be fixed with "
    "replace_link_target or remove_broken_link. The new target must already "
    "exist.\n"
    "- Generated projection pages are protected from link rewrites; if one "
    "needs non-navigation repair, call request_source_regeneration and "
    "describe the required regeneration.\n"
