---
page_id: coding-little-go-book-concurrency
page_kind: concept
summary: Concurrency: 86 statement(s) and 22 atom(s) from raw/coding_little_go_book.pdf.
sources: raw/coding_little_go_book.pdf
updated: 2026-06-29
domain: coding-little-go-book
category_path: concepts
projection_coverage: topic-coding-little-go-book-concurrency@0bbb4be92adca3f7f1de5c40ae6f2117
---

# Concurrency

What [[coding-little-go-book]] covers about concurrency:
...
### Technical frame 11: Chapter 6 - Concurrency / Channels / Buffered Channels
...
### Technical frame 14: Chapter 6 - Concurrency / Channels / Select
...
### Technical frame 15: Chapter 6 - Concurrency / Channels / Timeout
...
### Technical frame 22: Chapter 6 - Concurrency / Before You Continue
...
- [[coding-little-go-book-channel]] - shared statements and technical atoms: Channels shares source evidence from Chapter 6 - Concurrency / Channels: Our worker is simple. It waits until data is available then "processes" it. Dutifully, it does this in a loop, forever waiting for more data to process. To use this, ... [truncated]; Channels shares technical record from Chapter 6 - Concurrency / Channels: id int (39 shared statement(s), 12 shared atom(s))
- [[coding-little-go-book-synchronization]] - shared statements and technical atoms: Synchronization shares source evidence from Chapter 6 - Concurrency / Synchronization: Creating goroutines is trivial, and they are so cheap that we can start many; however, concurrent code needs to be coordinated. To help with this problem, Go provide ... [truncated]; Synchronization shares technical record from Chapter 6 - Concurrency / Synchronization: package main import ( "fmt" "time" ) var counter = 0 func main() { for i := 0; i < 20; i++ { go incr() (27 shared statement(s), 6 shared atom(s))
...
- [[coding-little-go-book-section-chapter-6-concurrency-55851f5e]] - source section: Chapter 6 - Concurrency shares source evidence from Chapter 6 - Concurrency: Go is often described as a concurrent-friendly language. The reason for this is that it provides a simple syntax over two powerful mechanisms: goroutines and channels.; Chapter 6 - Concurrency shares technical record from Chapter 6 - Concurrency / Goroutines: package main import ( "fmt" "time" ) func main() { fmt.Println("start") go process() time.Sleep(time.Millisecond * 10) // this is bad, don't do this! fmt.Println("done") } (86 shared statement(s), 22 shared atom(s))
